...
\subsection{a}
We can measure influence with the degree of nodes. For example, we can count total number of edges connected to a node to estimate the influence of an actor has on the network. By comparing network plot (above) and the actors who have the most degree (below), I am quite certain that I found an influential actor using degree. 
...
for(t in 1:18){
  temp <- graph.adjacency(as.matrix(network.mat[[t]]))
  degree <- degree(temp)
...
temp <- graph.adjacency(as.matrix(network.mat[[18]]))
degree <- degree(temp)
names(degree) <- as.character(gsub("\n", " ", names(degree)))
